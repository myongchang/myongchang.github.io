...
#define SM		1		// mode of market fluctuation (sMode)
...
#define PS		0.95		// probability that market size remains at the (t-1) level
...
int main()
{
	...
	for (t = 0; t < T; t++) {
		...
		fcpnout << cpn << std::endl;
		fcpmout << cpm << std::endl;
		...
		for (k = 0; k < M; k++) {
			if (m[k].status >= 2) {
				if (m[k].budget < I) {
					...
					// report the age of the firm at the time of exit if t>=3001
					if (t >= SL-1)
					{
						fsxageout << m[k].age << std::endl;
						countX = countX + 1;			// raise the exit counter
					}
	...
	// report the no. leadership changes and the no. exits for the replication
	fnleadout << countLead << std::endl;
	fnxout << countX << std::endl;

	}	// end the replication (tid) iteration
